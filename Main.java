...
class Main
{
	public static void main(String arg[]) throws Exception
	{
		...
		//Object creation of Cleaner class with reference cobj.
...
class Cleaner
{
	...
	public Cleaner(String name)												//constructor
	{
		...
		fdir = new File(name);
		//Accept Directory name and checks whether that directory is present or not.
		if(!fdir.exists())
		{
	...
	public void CleanDirectoryEmptyFile()										
	{
		...
		{
			//file.length() gives size of file.
			if(file.length() == 0)													
			{	
	...
	public void CleanDirectoryDuplicateFile() throws Exception
	{
		...
		try
		{
			...
			for(File file : filelist)
			{
				...
				// to get the hash bytes of cheksum i.e digest() method gives hashbytes
